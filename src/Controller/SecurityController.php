...
class SecurityController extends AbstractController
{
     ...
     * @Route("/")
     */
    public function void(AuthenticationUtils $authenticationUtils): Response
    {   
    ...
    /**
     * @Route("/statut-logout", name="app_statut_logout")
     */
    public function statut_logout(AuthenticationUtils $authenticationUtils): Response
    {
        
        return $this->render('security/statut_logout.html.twig');
    }
    ...
    public function forgotPassword(Request $request, UserPasswordEncoderInterface $passEnc, UserRepository $repoUser, EntityManagerInterface $manager, MailerInterface $mailerInterface)
    {
        if ($request->request->count() > 0) {
           ...
           if($user){
                ...
                $email = (new Email())
                ->from('assistant@dashboardsounds.com')
                ->to($adresse)
                ->cc('enac.fenitriniaina@gmail.com')
                //->bcc('bcc@example.com')
